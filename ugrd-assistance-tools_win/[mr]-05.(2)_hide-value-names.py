...
def mr_pc1_sc5_2(buffer, filesize):
    ...
    m_list = []
    m_item = {}
    m_item['offset'] = vk_cell['self'] # 'cell size' in the vk cell
    adjust = int((vk_cell['base'].value_name_size + 1) / BASE_ADJUST)
    edited_value = -(-vk_cell['base'].cell_size - adjust)
    ...
    m_item['info'] = "[original size] 0x%08X (@ 0x%08X) --> [manipulated size] 0x%08X (@ 0x%08X)" \
                     % (-vk_cell['base'].cell_size, m_item['offset'], -edited_value, m_item['offset'])                        
    m_item['info']+= "\n[Target 'vk_cell']\n: {}\n".format(vk_cell)
...
buffer = f.read()
manipulate(f, sys.argv[1], mr_pc1_sc5_2(buffer, len(buffer)))
f.flush()
f.close()
