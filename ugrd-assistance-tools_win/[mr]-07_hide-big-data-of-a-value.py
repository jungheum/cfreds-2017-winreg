...
def mr_pc1_sc7_1(buffer, filesize):
    ...
    # v1.5 only (if necessary)
    # if not (header.major_version == 1 and header.minor_version == 5):
    #    return []
...
buffer = f.read()
manipulate(f, sys.argv[1], mr_pc1_sc7_1(buffer, len(buffer)))
f.flush()
f.close()
