...
=============================================================================
    * Description
        CR type #2 - A hive bin randomly selected
...
def cr_type2(buffer, filesize):
    ...
    c_list = []
    for hbin in hbins:
        if not (hbin[0] < FIRST_HBIN_OFFSET + header.root_cell_offset and \
           FIRST_HBIN_OFFSET + header.root_cell_offset < hbin[0] + hbin[1]):
           c_list.append(hbin)
           
    c_list = [random.choice(c_list)] # a hive bin randomly selected
    return c_list
...
buffer = f.read()
overwrite(f, sys.argv[1], cr_type2(buffer, len(buffer)))
f.flush()
f.close()
