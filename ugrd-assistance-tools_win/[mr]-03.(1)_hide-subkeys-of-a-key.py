...
def mr_pc1_sc3_1(buffer, filesize):
    ...
    m_list = []
    m_item = {}
    m_item['offset'] = nk_cell['self'] + 0x18 # 'number of subkeys' in the nk cell
    edited_value = nk_cell['base'].number_of_subkeys - BASE_ADJUST
    m_item['data'] = edited_value.to_bytes(4, byteorder='little')
    m_item['info'] = "[original count] 0x%08X (@ 0x%08X) --> [manipulated count] 0x%08X (@ 0x%08X)" \
                     % (nk_cell['base'].number_of_subkeys, m_item['offset'], edited_value, m_item['offset'])
    m_item['info']+= "\n[Target 'nk_cell']\n: {}\n".format(nk_cell)
...
buffer = f.read()
manipulate(f, sys.argv[1], mr_pc1_sc3_1(buffer, len(buffer)))
f.flush()
f.close()
