...
def mr_pc3_sc1_1(buffer, filesize):
    ...
    while (1):
        ...
        if nk_cell['base'].number_of_values > 0:
            ...
            for offset in vl_cell['offsets']:
                ...
                # [condition] select a value cell which has DWORD_LE type data
                if vk_cell['base'].data_type == 0x00000004: # REG_DWORD_LE                
                    found = True
                    break
            if found is True:
                break
    ...
    edited_value = 0x00000008 # set to 8 (more than 4)
...
buffer = f.read()
manipulate(f, sys.argv[1], mr_pc3_sc1_1(buffer, len(buffer)))
f.flush()
f.close()
