...
=============================================================================
    * Description
        CR type #7 - First and last quarter
...
def cr_type7(buffer, filesize):
    ...
    for i in range (0, int(len(hbins)*0.25)):
        c_list.append(hbins[i])
    for i in range (int(len(hbins)*0.75), len(hbins)):
        c_list.append(hbins[i])
    return c_list
...
buffer = f.read()
overwrite(f, sys.argv[1], cr_type7(buffer, len(buffer)))
f.flush()
f.close()
