...
=============================================================================
    * Description
        CR type #4 - Fragments with hive bin header randomly selected
...
def cr_type4(buffer, filesize):
    ...
    for i in range (1, len(hbins)):
        c_list.append((hbins[i][0], FRAGMENT_HIVE_BLOCK))
           
    c_list = random.sample(c_list, int(len(hbins)/4)) # select 25% hive bin headers of the total
...
buffer = f.read()
overwrite(f, sys.argv[1], cr_type4(buffer, len(buffer)))
f.flush()
f.close()
