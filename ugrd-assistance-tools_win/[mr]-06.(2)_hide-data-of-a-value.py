...
def mr_pc1_sc6_2(buffer, filesize):
    ...
    # go to the data cell
    offset = hive.calc_hive_offset(vk_cell['base'].data_cell_offset)
    data_cell = hive.get_data(offset)
   
    # build manipulated item list (m_list)
    m_list = []
    m_item = {}
    m_item['offset'] = data_cell['self'] # 'cell size' in the data cell
    edited_value = -4 # set to 4
    ...
    m_item['info'] = "[original size] 0x%08X (@ 0x%08X) --> [manipulated size] 0x%08X (@ 0x%08X)" \
                     % (-data_cell['base'].cell_size, m_item['offset'], -edited_value, m_item['offset'])
    m_item['info']+= "\n[Target 'vk_cell']\n: {}\n".format(vk_cell)
...
buffer = f.read()
manipulate(f, sys.argv[1], mr_pc1_sc6_2(buffer, len(buffer)))
f.flush()
f.close()
