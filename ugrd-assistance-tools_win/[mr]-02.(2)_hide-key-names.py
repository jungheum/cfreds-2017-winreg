...
def mr_pc1_sc2_2(buffer, filesize):
    ...
    m_list = []
    m_item = {}
    m_item['offset'] = nk_cell['self'] # 'cell size' in the nk cell
    adjust = int((nk_cell['base'].key_name_size + 1) / BASE_ADJUST)
    edited_value = -(-nk_cell['base'].cell_size - adjust)
    ...
    m_item['info'] = "[original size] 0x%08X (@ 0x%08X) --> [manipulated size] 0x%08X (@ 0x%08X)" \
                     % (-nk_cell['base'].cell_size, m_item['offset'], -edited_value, m_item['offset'])
    m_item['info']+= "\n[Target 'nk_cell']\n: {}\n".format(nk_cell)
...
buffer = f.read()
manipulate(f, sys.argv[1], mr_pc1_sc2_2(buffer, len(buffer)))
f.flush()
f.close()
