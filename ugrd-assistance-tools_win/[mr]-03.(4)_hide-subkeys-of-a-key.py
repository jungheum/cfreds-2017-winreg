...
def mr_pc1_sc3_4(buffer, filesize):
    ...
    if nk_cell['base'].number_of_subkeys != sl_cell['base'].number_of_subkeys:
    ...
    for i in range(count-BASE_ADJUST, count):
        m_item = {}
        # 'subkey offsets' in the subkey-list cell
        m_item['offset'] = sl_cell['self'] + 0x08 + i*sl_cell['jump']
        ...
        m_item['info'] = "[original offset] 0x%08X (@ 0x%08X) --> [manipulated offset] 0x%08X (@ 0x%08X)" \
                         % (sl_cell['offsets'][i], m_item['offset'], edited_value, m_item['offset'])
...
buffer = f.read()
manipulate(f, sys.argv[1], mr_pc1_sc3_4(buffer, len(buffer)))
f.flush()
f.close()
