...
def mr_pc1_sc6_4(buffer, filesize):
    ...
    m_list = []
    m_item = {}
    m_item['offset'] = vk_cell['self'] + 0x10 # 'data type' in the vk cell
    edited_value = 0x00000001 # set to REG_SZ type
    m_item['data'] = edited_value.to_bytes(4, byteorder='little')
    m_item['info'] = "[original value] 0x%08X (@ 0x%08X) --> [manipulated value] 0x%08X (@ 0x%08X)" \
                     % (vk_cell['base'].data_type, m_item['offset'], edited_value, m_item['offset'])
    m_item['info']+= "\n[Target 'vk_cell']\n: {}\n".format(vk_cell)
...
buffer = f.read()
manipulate(f, sys.argv[1], mr_pc1_sc6_4(buffer, len(buffer)))
f.flush()
f.close()
